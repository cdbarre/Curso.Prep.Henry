...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  let frase='';
  const ultimo= palabras.length - 1;
  for (let index = 0; index < ultimo; index++) {
    frase+= palabras.shift() + ' '; 
  }
  frase+= palabras.shift(); 
  return frase;
}
...
function todosIguales(arreglo) {
  ...
  for (let index = 1; index < arreglo.length; index++) {
    if(valor !== arreglo[index]){ return false}
  }
  return true;
} 
...
function mayorACien(array) {
  ...
  let newArray=[];
  for (let index = 0; index < array.length; index++) {
    if(array[index] > 100){newArray.push(array[index])}
  }
  return newArray;
}
